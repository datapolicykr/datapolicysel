...
export async function POST(request:Request){
  try{
    ...
    if(!clean(body.clinicName)||!clean(body.region)||!clean(body.salesRep))return Response.json({error:"치과명, 지역, 오스템 영업사원을 입력해 주세요."},{status:400});
    const [saved]=await getDb().insert(surveys).values({
      ...
      linkedServices:JSON.stringify(linkedServices),inconveniences:JSON.stringify(inconveniences),contractType:clean(body.contractType),costItems:JSON.stringify(costItems),contractTerms:JSON.stringify(contractTerms),improvementItems:JSON.stringify(improvementItems),salesRep:clean(body.salesRep),terminalUsePeriod:clean(body.terminalUsePeriod),contractEndDate:clean(body.contractEndDate),monthlyCost:clean(body.monthlyCost),managementFee:clean(body.managementFee),currentPms:clean(body.currentPms),currentVanDealer:clean(body.currentVanDealer),contact:clean(body.contact),contactWhen:clean(body.contactWhen),linkFee:clean(body.linkFee),deliveryMethod:clean(body.deliveryMethod),fieldMemo:clean(body.fieldMemo),
